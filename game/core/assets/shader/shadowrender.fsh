...
float sampleFunc(vec2 coord, float r) {
...
void main(void) {
	...
	float center = sampleFunc(vec2(tc.x, tc.y), r);
	...
	sum += sampleFunc(vec2(tc.x - 4.0*blur, tc.y), r) * 0.05;
	sum += sampleFunc(vec2(tc.x - 3.0*blur, tc.y), r) * 0.09;
	sum += sampleFunc(vec2(tc.x - 2.0*blur, tc.y), r) * 0.12;
	sum += sampleFunc(vec2(tc.x - 1.0*blur, tc.y), r) * 0.15;
